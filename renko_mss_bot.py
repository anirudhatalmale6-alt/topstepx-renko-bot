"""
TopstepX Renko MSS Bot (ML + RL + Market Structure Shift)
============================================================
Strategy: Renko brick reversal + 15s MSS confirmation
- Build Renko bricks (0.25 pt brick size) from 30s price samples
- Bias: brick color flip sets direction (red->green = bullish, green->red = bearish)
- Entry: wait for Market Structure Shift on 15-second candles to confirm reversal
...
Usage:
    python renko_mss_bot.py --symbols "NQ:1" --tick-interval 1
"""
...
class SymbolState:
    def __init__(self, symbol: str, base_qty: int, ntfy_topic: str,
        ...
        self._pending_signal = None  # "LONG" or "SHORT" — set by Renko, waiting for MSS confirm
    ...
    def tick(self, price: float, ts: float = None):
        ...
        if new_bricks:
            for brick in new_bricks:
                ...
                # Brick flip = set pending signal, wait for 15s MSS confirmation
...
class RenkoMSSBot:
    def __init__(self, symbol_configs: list, tg_token: str = "",
        ...
        self.suite = None
        self.states = {}
        self.state_file = os.path.join(os.getcwd(), "bot_state_mss.json")
    ...
    async def run(self):
        ...
        print(f"[BOT] Renko MSS Bot starting...")
        ...
        print(f"[BOT] Trading LIVE — Renko MSS strategy ({', '.join(symbols)})")
        print(f"[BOT] Watchdog: {WATCHDOG_TIMEOUT}s timeout")
        print(f"[BOT] Press Ctrl+C to stop")

        for sym, st in self.states.items():
            msg = (f"STATUS|Renko MSS Bot started (ML+RL)\n"
                   f"Account: {acct}\n"
...
def main():
    parser = argparse.ArgumentParser(description="TopstepX Renko MSS Bot")
    ...
    print(f"[BOT] Renko MSS Bot (ML + RL)")
    print(f"[BOT] Brick: {BRICK_SIZE}pt | TP: ${DCA_TP_DOLLARS} | RL: SL/trail adaptive")
    print(f"[BOT] ENTRY: brick color flip -> 15s MSS confirmation -> enter")
    ...
    while not stopped:
        ...
        try:
            ...
            send_telegram(args.tg_token, args.tg_chat,
                          f"STATUS|Renko MSS bot crashed, restarting in {retry_delay}s")
