"""
TopstepX BB Breakout Bot (LIVE)
================================
Strategy: Bollinger Band(20, 1.5) breakout on 15-second candles
- Compute BB(20, SMA, close, 1.5 StdDev, ddof=0) matching TradingView
- When a candle fully closes below lower BB → armed for LONG
- Price breaks back above lower BB → ENTER LONG
- When a candle fully closes above upper BB → armed for SHORT
- Price breaks back above upper BB → ENTER SHORT
- Default SL: breakout failure (candle closes back outside BB)
- RL learns to adjust SL/TP over time from discrete tiers
- Re-enter if price tries to break again after exit

Usage:
    python bb_breakout_bot.py --symbols "NQ:1" --tick-interval 1
"""
...
BB_LENGTH = 20
BB_MULT = 1.5
CANDLE_SECONDS = 15  # 15-second candles
...
class CandleBuilder:
    """Builds fixed-interval candles from tick data (second-level precision)."""
...
EARLY_EXIT_COOLDOWN = 10  # shorter for 15s candles
SLTP_WARMUP = 40  # trades before RL adjusts SL/TP


class TradeML:
    """Q-learning RL for BB breakout with SL/TP tier learning."""
    ...
    def record_trade(self, features: dict, pnl: float, source: str = "live",
        ...
        total = len(self.trades)
        sl_str = f"SL={SL_TIERS[sl_idx]}pts" if sl_idx < len(SL_TIERS) and SL_TIERS[sl_idx] > 0 else "SL=candle"
...
class SymbolState:
    def __init__(self, symbol: str, base_qty: int, ntfy_topic: str,
        ...
        self.armed_level = None      # BB level to break back through
        self.armed_strength = 0.0    # how far candle closed outside BB
        self._last_rl_check = 0.0    # throttle RL checks on armed breakouts
        self._rl_check_cooldown = 3  # seconds between RL re-checks on same arm
        ...
        # RL
        self.ml = TradeML(os.path.join(os.getcwd(), f"rl_state_{symbol}_bb.json"))
    ...
    def extract_features(self) -> dict:
        ...
        return self.ml.extract_features(
            ...
            armed_strength=self.armed_strength)
    ...
    def tick(self, price: float, ts: float = None):
        """Process a tick. BB breakout strategy on 15-second candles."""
        if ts is None:
            ts = time.time()
        ...
        # --- Tick-level: check if armed breakout triggers entry ---
        # Re-entry: arm stays active after skip or exit so price can re-trigger
        if self.armed_direction is not None and self.position == 0 and in_trade_session():
            if ts - self._last_rl_check < self._rl_check_cooldown:
                pass  # throttle RL checks
            elif self.armed_direction == "long" and self.armed_level is not None:
                if price > self.armed_level:
                    now_str = datetime.now(ET).strftime("%H:%M:%S")
                    print(f"[{now_str}] [{self.symbol} BREAKOUT] LONG trigger: "
                          f"price {price:.2f} > lower BB {self.armed_level:.2f}")

                    features = self.extract_features()
                    action_str, _, reason = self.ml.should_skip(features)
                    self._last_rl_check = ts
                    ...
                    if action_str in ("enter", "flip"):
                        ...
                        # Keep armed — price may drop back below and break again
                        print(f"  Breakout SKIPPED by RL (arm stays active for re-entry)")

            elif self.armed_direction == "short" and self.armed_level is not None:
                if price < self.armed_level:
                    now_str = datetime.now(ET).strftime("%H:%M:%S")
                    print(f"[{now_str}] [{self.symbol} BREAKOUT] SHORT trigger: "
                          f"price {price:.2f} < upper BB {self.armed_level:.2f}")
        ...
        # --- Candle close: update BB, check arm/stop ---
        ...
        # --- If in position: check candle failure stop ---
        if self.position != 0 and self.active_sl_pts == 0:
            # Default SL: candle closes back outside BB = breakout failure
            if self.position == 1 and candle_close < bb["lower"]:
                print(f"[{now_str}] [{self.symbol} CANDLE-FAIL] LONG: candle closed {candle_close:.2f} "
                      f"< lower BB {bb['lower']:.2f}")
                self._pending_rl = {"features": self.entry_features,
                                    "entry_action": self.entry_action,
                                    "sl_idx": self.active_sl_idx,
                                    "tp_idx": self.active_tp_idx,
                ...
                # Re-arm: candle closed below BB again → ready for next break
                strength = bb["lower"] - candle_close
                self.armed_direction = "long"
                self.armed_level = bb["lower"]
                self.armed_strength = strength
                self._last_rl_check = 0.0
                print(f"  Re-armed LONG (strength: {strength:.2f}pts)")
                return actions
            elif self.position == -1 and candle_close > bb["upper"]:
                print(f"[{now_str}] [{self.symbol} CANDLE-FAIL] SHORT: candle closed {candle_close:.2f} "
                      f"> upper BB {bb['upper']:.2f}")
                self._pending_rl = {"features": self.entry_features,
                                    "entry_action": self.entry_action,
                                    "sl_idx": self.active_sl_idx,
                                    "tp_idx": self.active_tp_idx,
                                    "mae": self.trade_mae, "mfe": self.trade_mfe}
                actions.append(("flatten", price, "CANDLE_FAIL"))
                # Re-arm: candle closed above BB again → ready for next break
                strength = candle_close - bb["upper"]
                self.armed_direction = "short"
                self.armed_level = bb["upper"]
                self.armed_strength = strength
                self._last_rl_check = 0.0
                print(f"  Re-armed SHORT (strength: {strength:.2f}pts)")
                return actions

        # --- If in position: RL hold check on candle close ---
        if self.position != 0:
            features = self.extract_features()
            action_str, _, reason = self.ml.should_skip(features)
            direction = "LONG" if self.position == 1 else "SHORT"
            ...
            if action_str == "flip":
                ...
                # RL says skip while in position = consider closing
        ...
        if self.position == 0:
            ...
            if candle_high_body < bb["lower"]:
                ...
                # (price moved back inside without the tick-level break being caught,
                #  or we want to re-arm on the next close outside)
                pass
    ...
    def restore_state(self, state: dict, position_ttl: int = 600) -> bool:
        ...
        if age < position_ttl:
            ...
            self.armed_direction = state.get("armed_direction")
            self.armed_level = state.get("armed_level")
            self.armed_strength = state.get("armed_strength", 0.0)
...
class BBBreakoutBot:
    def __init__(self, symbol_configs: list, tg_token: str = "",
        ...
        self.suite = None
        self.states = {}
        self.state_file = os.path.join(os.getcwd(), "bot_state_bb.json")
    ...
    async def run(self):
        ...
        print(f"[BOT] BB Breakout Bot starting...")
        print(f"[BOT] Strategy: BB({BB_LENGTH}, {BB_MULT}) breakout on {CANDLE_SECONDS}s candles")
        ...
        print(f"[BOT] Trading LIVE — BB Breakout strategy ({', '.join(symbols)})")
        ...
        for sym, st in self.states.items():
            stats = st.ml.stats()
            msg = (f"STATUS|BB Breakout Bot started\n"
                   f"Account: {acct}\n"
        ...
        # Main tick loop
        self.last_tick_time = time.time()
        save_interval = 15  # more frequent saves for fast strategy
        last_save = time.time()
        ...
        while self.running:
            ...
            if now - last_status > status_interval:
                for sym, st in self.states.items():
                    ...
                    armed_str = f"Armed: {st.armed_direction}" if st.armed_direction else "Armed: none"
                    print(f"  [{sym} @ {datetime.now(ET).strftime('%H:%M:%S')}]")
                    print(f"    Price: {st.last_price:.2f} | {bb_str}")
                    print(f"    {armed_str} | Position: {pos_str} x{st.contracts_held} | "
...
def main():
    parser = argparse.ArgumentParser(description="TopstepX BB Breakout Bot")
    ...
    print(f"[BOT] BB Breakout Bot")
    print(f"[BOT] BB({BB_LENGTH}, {BB_MULT}) on {CANDLE_SECONDS}s candles")
    print(f"[BOT] ENTRY: candle closes outside BB -> price breaks back -> enter")
    ...
    while not stopped:
        ...
        try:
            ...
            send_telegram(args.tg_token, args.tg_chat,
                          f"STATUS|BB Breakout bot crashed, restarting in {retry_delay}s")
